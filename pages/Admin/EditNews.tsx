...
const EditNews: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If no main image selected and no existing image, use default logo_tpq.png
      let finalMainImage: File | undefined = mainImage || undefined;
      if (!finalMainImage && !existingMainImage) {
        try {
          const res = await fetch('/logo_tpq.png');
          const blob = await res.blob();
          finalMainImage = new File([blob], 'logo_tpq.png', { type: blob.type || 'image/png' });
        } catch {
